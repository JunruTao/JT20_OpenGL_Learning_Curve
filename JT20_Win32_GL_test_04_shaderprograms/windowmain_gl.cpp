...
void main()
{
	gl_FragData[0] = vec4(Scale * 3, Scale * 3, 0.3, 1.0);
}
);
	...
	int iCmdShow)
{
	...
	MyVertex pvertex[8] = 
	{ 
		{-bsize,bsize,bsize},{bsize,bsize,bsize},
		{bsize,-bsize,bsize},{-bsize,-bsize,bsize},
		{-bsize-0.2f,bsize+0.1f,-bsize},
		{bsize - 0.2f,bsize + 0.1f,-bsize},
		{bsize - 0.2f,-bsize + 0.1f,-bsize},
		{-bsize - 0.2f,-bsize + 0.1f,-bsize} 
	};
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(point_indices2), point_indices2, GL_STATIC_DRAW);
	...
	while (running)
	{
		...
		glUseProgram(program1);
			//get the uniform Scale Variable id from shader program
		...
		static GLint locScale2 = glGetUniformLocation(program2, "Scale");
			glUniform1f(locScale2, Scale1);
			glDrawElements(GL_QUADS, sizeof(point_indices2) / sizeof(short), GL_UNSIGNED_SHORT, 0);
		glUseProgram(0);
